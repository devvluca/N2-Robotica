...
│                     SIMULAÇÃO (PyBullet)                     │
├─────────────────────────────────────────────────────────────┤
│  Sensores Simulados          │  Atuadores                   │
│  • Encoder virtual (ângulo)  │  • Motores nas juntas        │
│  • Sensor de torque          │  • Controle torque/velocidade│
├─────────────────────────────────────────────────────────────┤
│                     CONTROLE PID                             │
│  • Mede diferença: (ângulo_ref - ângulo_atual)              │
│  • Converte erro em torque aplicado                          │
│  • Controle individual por junta                             │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│                     SIMULAÇÃO (PyBullet)                     │
├─────────────────────────────────────────────────────────────┤
│  Ambiente                    │  Robô Diferencial            │
│  • Planta 2.5x2.5 metros     │  • Base circular (ø 32cm)    │
│  • Piso plano + obstáculos   │  • 5 sensores ultrassônicos  │
│  • Paredes limitantes        │  • Rodas independentes       │
├─────────────────────────────────────────────────────────────┤
│  Sensores                    │  Atuadores                   │
│  • Distância (5 raycasts)    │  • Motor esquerdo (PID)      │
│  • Pose estimada (x,y,θ)     │  • Motor direito (PID)       │
│  • Velocidade atual          │  • Controle diferencial      │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│  📝 Log de trajetória e tempo por célula                    │
│  🧠 Algoritmo de otimização: evitar áreas já limpas         │
│  💾 Persistência: salvar/carregar mapas entre execuções     │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    SUPERVISÓRIO (Node-RED)                   │
├─────────────────────────────────────────────────────────────┤
│  📊 Trajetória percorrida (plot)                            │
│  ⏱️ Tempo total vs. área coberta                            │
...
```
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│   SWEEPING   │───▶│ TURNING_END  │───▶│   SWEEPING   │
│  (Varredura) │    │ (Curva em U) │    │  (Próx.faixa)│
└──────────────┘    └──────────────┘    └──────────────┘
       │                                       │
       ▼                                       ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│   AVOIDING   │◀──▶│  REVERSING   │    │   SEEKING    │
│   (Desvio)   │    │    (Ré)      │    │(Busca áreas) │
└──────────────┘    └──────────────┘    └──────────────┘
                                               │
                                               ▼
                                        ┌──────────────┐
...
```bash
git clone https://github.com/devvluca/N2-Robotica.git
cd N2-Robotica
```
...
2. Menu (☰) → **Import**
3. Selecione o arquivo `flow.json` ou `node_red_flow.json` do projeto desejado
4. Clique em **Deploy**
...
- **Lucas Almeida** - [@devvluca](https://github.com/devvluca)
